...
const Table = (
    ...
    const tableHead = ['All', 'Task ID', 'Title', 'Priority', 'Start Time', 'End Time', 'Time Left', 'Edit'];
...
export const TableData = ({ data,task }: { data: string | number | React.ReactElement,task?:BTaskSchemaType }) => {
    ...
    return (
        <td className="p-4 border-b border-accent">
            <p className={`block font-sans text-md antialiased font-normal leading-norma ${task?.taskStatus === "FINISHED" ? 'text-zinc-600' : 'text-whitish'}`}>
                {data}
            </p>
        </td>
    )
